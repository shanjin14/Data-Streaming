...
spark = SparkSession.builder.appName("customer-risks-joined-stream").getOrCreate()
...
casted_Stream_DF = raw_Stream_DF.selectExpr("cast(key as string) key", "cast(value as string) value")
...
casted_Stream_DF.withColumn("value",from_json("value",redisMessageSchema))\
        .select(col('value.*')) \
...
encoded_Stream_DF = spark.sql("select key, zSetEntries[0].element as encodedCustomer from RedisData")
decoded_Stream_DF = encoded_Stream_DF.withColumn("customer", unbase64(encoded_Stream_DF.encodedCustomer).cast("string"))
...
emailAndBirthYearStreamingDF = emailAndBirthDayStreamingDF.selectExpr("email", "split(birthDay,'-')[0] as birthYear")


# TO-DO: using the spark application object, read a streaming dataframe from the Kafka topic stedi-events as the source
# Be sure to specify the option that reads all the events from the topic including those that were published before you started the spark stream
raw_stedi_Stream_DF = spark                          \
    .readStream                                          \
    .format("kafka")                                     \
    .option("kafka.bootstrap.servers", "localhost:9092") \
...
# TO-DO: cast the value column in the streaming dataframe as a STRING 
casted_stedi_Stream_DF = raw_stedi_Stream_DF.selectExpr("cast(key as string) key", "cast(value as string) value")
...
casted_stedi_Stream_DF.withColumn("value",from_json("value",stedieventSchema))\
        .select(col('value.*')) \
        .createOrReplaceTempView("CustomerRisk")


# TO-DO: execute a sql statement against a temporary view, selecting the customer and the score from the temporary view, creating a dataframe called customerRiskStreamingDF
customerrisk_stream_DF=spark.sql("select customer,score from CustomerRisk")

# TO-DO: join the streaming dataframes on the email address to get the risk score and the birth year in the same dataframe
output_stream_DF = customerrisk_stream_DF.join(emailAndBirthYearStreamingDF, expr( """
   customer=email
"""
))
...
#output_stream_DF.writeStream.outputMode("append").format("console").start().awaitTermination()


output_stream_DF.selectExpr("CAST(customer AS STRING) AS key", "to_json(struct(*)) AS value")\
    .writeStream \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "localhost:9092")\
    .option("topic", "customer-risk-events")\
