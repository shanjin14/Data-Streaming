...
class Station(Producer):
    ...
    def __init__(self, station_id, name, color, direction_a=None, direction_b=None):
        ...
        #
        #
        topic_name = f"com.cta.stations.arrival.{station_name}" # TODO: Come up with a better topic name
        super().__init__(
    ...
    def run(self, train, direction, prev_station_id, prev_direction):
        ...
        print("publish to ",self.topic_name,self.station_id,train.train_id,direction,self.color.name,train.status.name,prev_station_id,prev_direction)
        self.producer.produce(
            topic=self.topic_name,
            key={"timestamp": self.time_millis()},
            key_schema=Station.key_schema,
            value=asdict(ArrivalEvent(self.station_id,train.train_id,direction,self.color.name,train.status.name,prev_station_id,prev_direction)),
            value_schema = Station.value_schema
        )
