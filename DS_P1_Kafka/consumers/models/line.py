...
class Line:
    ...
    def process_message(self, message):
        ...
        if "com.cta.database.stations_transformed" in message.topic(): # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif "com.cta.stations.arrival" in message.topic(): # Set the conditional to the arrival topic
